...
///The point size of each block sprite - in our case 20.0 x 20.0 - the lower of the available resolution options for each block image. We also declare a layer position which will give us an offset from the edge of the screen.
let BlockSize:CGFloat = 20.0

///This variable will represent the slowest speed at which our shapes will travel. We've set it to 600 milliseconds, which means that every 6/10ths of a second, our shape should descend by one row.
let TickLengthLevelOne = NSTimeInterval(600)

class GameScene: SKScene {
    
    /*
        w"'ve introduced a couple of SKNodes which can be thought of as superimposed layers of activity within our scene. The gameLayer sits above the background visuals and the shapeLayer sits atop that
    */
    ...
    */
    ...
    /*
        Accessor methods to let external classes stop and start the ticking process, something we'll make use of later in order to keep pieces from falling at key moments.
    */
    func startTicking() {
        lastTick = NSDate()
    }
    ...
    func addPreviewShapeToScene(shape: Shape, completion: () -> ()) {
        ...
        for (idx, block) in enumerate(shape.blocks){
            ...
            */
            ...
            /*
                We use our convenient pointForColumn(Int, Int) method to place each block's sprite in the proper location. We start it at row - 2, such that the preview piece animates smoothly into place from a higher location.
            */
    ...
    /// You can see that we take in precisely the tuple data which Swiftris returns each time a line is removed. This will ensure that GameViewController need only pass those elements to GameScene in order for them to animate properly.
    func animateCollapsingLines(linesToRemove: Array<Array<Block>>, fallenBlocks: Array<Array<Block>>, completion:() -> ()) {
        ...
        for (rowIdx, row) in enumerate(linesToRemove) {
            for (blockIdx, block) in enumerate(row) {
                ...
                /// We choose beginning and starting angles, these are clearly in radians and if your trigonometry is as rough as ours was when we wrote this, a circle in radian degrees
